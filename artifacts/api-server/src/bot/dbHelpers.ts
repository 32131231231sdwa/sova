...
export async function addXP(
  ...
  if (user.level >= 50) return { leveledUp: false, newLevel: 50 };

  const newXp = user.xp + amount;
  const nextLevelXp = getLevelXP(user.level + 1);
  const questsForNextLevel = QUESTS.filter(
    (q) => q.forLevel === user.level + 1,
  );
  const allQuestsDone =
...
export async function getFamily(telegramId: number): Promise<OwlFamily | null> {
  const rows = await db
    ...
    .where(
      or(
        eq(owlFamilies.user1Id, telegramId),
        eq(owlFamilies.user2Id, telegramId),
      ),
    )
    .limit(1);
  return rows[0] ?? null;
...
export async function getFamilyUsers(
  family: OwlFamily,
): Promise<[OwlUser, OwlUser]> {
  const [u1] = await db
    .select()
    .from(owlUsers)
    .where(eq(owlUsers.telegramId, family.user1Id))
    .limit(1);
  const [u2] = await db
    .select()
    .from(owlUsers)
    .where(eq(owlUsers.telegramId, family.user2Id))
    .limit(1);
  return [u1!, u2!];
}
...
export async function createFamily(
  ...
  const [fam] = await db
    .insert(owlFamilies)
    .values({ user1Id, user2Id, familyName })
    .returning();
  return fam!;
}
...
export async function updateDuel(
  duelId: number,
  data: Partial<typeof owlDuels.$inferInsert>,
) {
...
export async function createDuel(
  ...
  const [duel] = await db
    .insert(owlDuels)
    .values({ challengerId, challengedId, chatId })
    .returning();
  return duel!;
}
...
export async function getTopByFeathers(limit = 10) {
  return db
    .select()
    .from(owlUsers)
    .orderBy(desc(owlUsers.feathers))
    .limit(limit);
}
...
export async function getTopByLevel(limit = 10) {
  return db
    .select()
    .from(owlUsers)
    .orderBy(desc(owlUsers.level), desc(owlUsers.xp))
    .limit(limit);
}
...
export async function getPendingInvite(fromId: number, toId: number) {
  const rows = await db
    ...
    .where(
      and(
        eq(owlFamilyInvites.fromId, fromId),
        eq(owlFamilyInvites.toId, toId),
      ),
    )
    .limit(1);
  return rows[0] ?? null;
...
export async function getInviteForUser(toId: number) {
  const rows = await db
    ...
    .where(
      and(
        eq(owlFamilyInvites.toId, toId),
        sql`${owlFamilyInvites.expiresAt} > now()`,
      ),
    )
    .limit(1);
  return rows[0] ?? null;
...
export async function createFamilyInvite(
  ...
  await db
    ...
    .where(
      or(
        eq(owlFamilyInvites.fromId, fromId),
        eq(owlFamilyInvites.toId, fromId),
      ),
    );
  const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
  const [inv] = await db
    .insert(owlFamilyInvites)
    .values({ fromId, toId, chatId, expiresAt })
    .returning();
  return inv!;
}
...
export async function getUserQuest(telegramId: number, questId: string) {
  const [q] = await db
    ...
    .from(owlQuests)
    .where(
      and(eq(owlQuests.telegramId, telegramId), eq(owlQuests.questId, questId)),
    )
    .limit(1);
  return q ?? null;
}
...
export async function upsertQuestProgress(
  ...
  if (!existing) {
    await db
      .insert(owlQuests)
      .values({ telegramId, questId, target, progress: increment });
    return;
  }
  ...
  if (completed) {
    await db
      .update(owlUsers)
      .set({
        questsCompleted: sql`array_append(${owlUsers.questsCompleted}, ${questId})`,
      })
      .where(eq(owlUsers.telegramId, telegramId));
  }
}
...
export async function getAllUserQuests(telegramId: number) {
  return db
    .select()
    .from(owlQuests)
    .where(eq(owlQuests.telegramId, telegramId));
}
